...
def add_random_noise(affinity_mat, noise_level=0.01):
    noise = np.random.normal(0, noise_level, affinity_mat.shape)
    noisy_affinity_mat = affinity_mat + noise
    return noisy_affinity_mat
...
def process_data_with_augmentation(affinity_mat, dataset, num_pos, pos_threshold, noise_level=0.01, num_augmented_samples=5):
    dataset_path = 'data/' + dataset + '/'
    ...
    augmented_train_datasets = [train_dataset]
    for _ in range(num_augmented_samples):
        # 选择一种变换方法
        transformation = np.random.choice(['transpose', 'flip_horizontal', 'flip_vertical', 'noise'])
        if transformation == 'transpose':
            augmented_affinity_mat = transpose_matrix(affinity_mat)
        elif transformation == 'flip_horizontal':
            augmented_affinity_mat = flip_matrix_horizontal(affinity_mat)
        elif transformation == 'flip_vertical':
            augmented_affinity_mat = flip_matrix_vertical(affinity_mat)
        elif transformation == 'noise':
            augmented_affinity_mat = add_random_noise(affinity_mat, noise_level)

        train_affinity_mat = np.zeros_like(augmented_affinity_mat)
        train_affinity_mat[train_rows, train_cols] = train_Y
        affinity_graph, drug_pos, target_pos = get_affinity_graph(dataset, train_affinity_mat, num_pos, pos_threshold)
        augmented_train_dataset = DTADataset(drug_ids=train_rows, target_ids=train_cols, y=train_Y)
        augmented_train_datasets.append(augmented_train_dataset)

    return augmented_train_datasets, test_dataset, affinity_graph, drug_pos, target_pos
